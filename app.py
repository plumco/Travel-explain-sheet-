...
.html-header { background:#1e40af; color:white; padding:18px 16px; text-align:center; margin:0 -2rem 18px -2rem; }
.html-header .brand { display:inline-block; margin-bottom:8px; padding:5px 10px; border:1px solid rgba(255,255,255,.55); border-radius:999px; font-size:12px; letter-spacing:.3px; background:rgba(255,255,255,.12); }
.html-header h1 { margin:0; font-size:22px; line-height:1.1; }
.html-header p { margin:6px 0 0; font-size:14px; opacity:.9; }
div[data-testid="stForm"], .card-box { background:white; border-radius:14px; padding:16px; margin-bottom:16px; box-shadow:0 4px 14px rgba(15,23,42,.08); border:0; }
...
.err-text { color:#b91c1c; font-weight:700; font-size:13px; }
label, [data-testid="stWidgetLabel"] p { font-size:12px !important; font-weight:700 !important; color:#374151 !important; margin-bottom:5px !important; }
input, textarea, select { border:1px solid #cbd5e1 !important; border-radius:8px !important; font-size:14px !important; background:white !important; }
.stButton>button, .stDownloadButton>button { border:0; border-radius:8px; padding:10px 14px; font-size:14px; font-weight:800; min-height:42px; }
...
.summary-box { background:#eff6ff; border:1px solid #bfdbfe; border-radius:12px; padding:12px; text-align:center; min-height:74px; }
...
def get_gsheet_config():
    sheet_name = get_secret_value("SHEET_NAME", "Travel Expense Data")
    ...
    return sheet_name, worksheet_name, spreadsheet_id


def get_google_worksheet():
    """Return (worksheet, error_text). No local fallback. Shows real error."""
    try:
        import gspread
        from google.oauth2.service_account import Credentials
    except Exception as e:
        return None, "Missing package: install gspread and google-auth. Details: " + repr(e)

    try:
        if "gcp_service_account" not in st.secrets:
            return None, "Missing [gcp_service_account] in Streamlit Secrets."
        if "gsheets" not in st.secrets:
            return None, "Missing [gsheets] section in Streamlit Secrets."

        _sheet_name, worksheet_name, spreadsheet_id = get_gsheet_config()
        if not spreadsheet_id:
            return None, "Missing [gsheets] spreadsheet_id in Streamlit Secrets."
        ...
        try:
            ...
            ws = spreadsheet.add_worksheet(title=worksheet_name, rows=1000, cols=len(SHEET_COLUMNS) + 5)
        ...
        if not values:
            ...
            existing = values[0]
            missing = [c for c in SHEET_COLUMNS if c not in existing]
            if missing:
...
def read_entries():
    ...
    if err:
        return pd.DataFrame(columns=SHEET_COLUMNS), err
    try:
        ...
        for col in SHEET_COLUMNS:
            if col not in df.columns:
                df[col] = ""
        return df[SHEET_COLUMNS].fillna(""), None
    except Exception:
        return pd.DataFrame(columns=SHEET_COLUMNS), traceback.format_exc()
...
def append_entry(row):
    ...
    if err:
        return False, err
    try:
        clean = {col: row.get(col, "") for col in SHEET_COLUMNS}
        ws.append_row([clean[col] for col in SHEET_COLUMNS], value_input_option="USER_ENTERED")
        return True, None
    except Exception:
...
def rewrite_entries(df):
    ...
    try:
        ...
        ws.clear()
        ws.update("A1", [SHEET_COLUMNS] + df.astype(str).values.tolist())
        return True, None
    except Exception:
        return False, traceback.format_exc()
...
def update_entry(entry_id, updated_row):
    ...
    if not mask.any():
        return False, "Entry ID not found in Google Sheet."
    for col in SHEET_COLUMNS:
        df.loc[mask, col] = updated_row.get(col, "")
    return rewrite_entries(df)
...
def generate_excel(df, employee):
    ...
    for r, (_, row) in enumerate(filtered.iterrows(), start=data_start):
        ...
        ws.cell(r, 9).value = money(row.get("vehicle"))
...
def default_employee():
    return {
        ...
        "location": "Pune",
        "reportMonthText": "01st April 2026 To 30th April 2026",
    }
...
def render_connection_status():
    ws, err = get_google_worksheet()
    if err:
        st.markdown('<p class="err-text">Storage: Google Sheets not connected.</p>', unsafe_allow_html=True)
...
def main():
    ...
    sheet_name, worksheet_name, spreadsheet_id = get_gsheet_config()
    st.caption(f"Sheet ID: {spreadsheet_id or 'Not set'} | Worksheet: {worksheet_name}")
    ...
    employee = {"empName": emp_name, "designation": designation, "location": location, "reportMonthText": report_text}
    ...
    with st.form("entry_form", clear_on_submit=True):
        ...
        vehicle = c4.number_input("2 Wheeler / 4 Wheeler", min_value=0.0, value=0.0, step=1.0)
        ...
        if save:
            if not make_str(from_loc) or not make_str(to_loc):
                ...
                total = sum(money(x) for x in [toll, fuel, lodging, food, tel, courier, rikshaw, vehicle])
                ok, err = append_entry({
                    "id": str(uuid.uuid4())[:8],
    ...
    df, read_err = read_entries()
    if read_err:
        st.error("Cannot read Google Sheet data.")
        st.code(read_err)
        filtered = pd.DataFrame(columns=SHEET_COLUMNS)
    else:
        filtered = df.copy()
        if emp_name:
            filtered = filtered[filtered["empName"].astype(str) == emp_name]
        if report_text:
            filtered = filtered[filtered["reportMonthText"].astype(str) == report_text]
        ...
        try:
            ...
            with b1:
                st.download_button(
                    ...
                    data=excel_bytes,
                    file_name=f"Travelling_Expenses_{emp_name.replace(' ', '_').replace('.', '')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
                )
        except Exception as e:
            b1.error("Excel generation error.")
            b1.code(traceback.format_exc())
    else:
        b1.error("Excel template file missing: Travelling Expenses Sheet.xlsx")

    with b2:
        st.download_button("Backup CSV", data=filtered.to_csv(index=False).encode("utf-8"), file_name="travel_expense_backup.csv", mime="text/csv", use_container_width=True)
    with b3:
    ...
    if filtered.empty:
        ...
        show_cols = ["date", "from", "to", "company", "contact", "invoice", "toll", "fuel", "vehicle", "lodging", "food", "tel", "courier", "rikshaw", "total", "remarks", "id"]
        display = filtered[[c for c in show_cols if c in filtered.columns]]
    ...
    if filtered.empty:
        ...
        for _, r in filtered.iterrows():
            options.append((f"{r.get('date','')} | {r.get('from','')} to {r.get('to','')} | {r.get('company','')} | ID: {r.get('id','')}", str(r.get("id", ""))))
        ...
        if not selected_rows.empty:
            ...
            with st.form("edit_form"):
                ...
                edit_vehicle = e4.number_input("2 Wheeler / 4 Wheeler", min_value=0.0, value=money(selected.get("vehicle")), step=1.0, key="edit_vehicle")
                e5, e6 = st.columns(2)
                ...
                if update_clicked:
                    total = sum(money(x) for x in [edit_toll, edit_fuel, edit_lodging, edit_food, edit_tel, edit_courier, edit_rikshaw, edit_vehicle])
                    ok, err = update_entry(selected_id, {
                        "id": selected_id,
                        "empName": make_str(emp_name), "designation": make_str(designation), "location": make_str(location), "reportMonthText": make_str(report_text),
                        "date": edit_date.strftime("%d-%m-%Y"), "from": make_str(edit_from), "to": make_str(edit_to), "vehicle": edit_vehicle,
                        "company": make_str(edit_company), "contact": make_str(edit_contact), "invoice": make_str(edit_invoice),
                        "toll": edit_toll, "fuel": edit_fuel, "lodging": edit_lodging, "food": edit_food, "tel": edit_tel, "courier": edit_courier, "rikshaw": edit_rikshaw,
                        "remarks": make_str(edit_remarks), "total": total, "created_at": make_str(selected.get("created_at")) or current_stamp(),
                    })
                    if ok:
                        ...
                        st.code(err)
                if delete_clicked:
                    ok, err = delete_entry(selected_id)
                    if ok:
